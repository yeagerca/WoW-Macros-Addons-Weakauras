...
function PallyPower:PerformCycle(name, class, skipzero)
	...
	for test = cur+1, PallyPower.BLESSINGS_COUNT do
	...
	if cur == PallyPower.BLESSINGS_COUNT then
...
function PallyPower:PerformCycleBackwards(name, class, skipzero)
	...
	if not PallyPower_Assignments[name][class] then
		cur=PallyPower.BLESSINGS_COUNT
	else
		cur=PallyPower_Assignments[name][class]
		if cur == 0 or skipzero and cur == 1 then cur = PallyPower.BLESSINGS_COUNT end
	end
...
function PallyPower:CanBuff(name, test)
	if test==PallyPower.BLESSINGS_COUNT then
		return true
	end
